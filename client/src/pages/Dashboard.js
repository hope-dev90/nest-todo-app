...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { label: 'This Week',    value: notes.filter(n => {
      ...
      }).length,                                                          color: 'green',  icon: '📆' },
    { label: 'Updated Today', value: notes.filter(n => {
        const d = new Date(n.updatedAt);
        return d.toDateString() === new Date().toDateString();
      }).length,                                                          color: 'red',    icon: '✏️' },
  ];
  ...
  return (
    <AppLayout>
      ...
      <div className="hero-banner">
        ...
        <p>Check your notes and schedule for {format(new Date(), 'EEEE, MMMM d')}</p>
        <button className="btn-outline-white" onClick={() => setModalOpen(true)}>
          + New Note
        </button>
      </div>
      ...
      <div className="stats-grid">
        {stats.map((s, i) => (
          <div className={`stat-card ${s.color}`} key={s.label} style={{ animationDelay: `${i * 0.07}s` }}>
      ...
      <div className="two-col">
        ...
        <div className="card">
          <div className="section-header">
            <h3>Recent Notes</h3>
            <button className="btn-sm btn-ghost" onClick={() => navigate('/notes')}>View all</button>
          </div>
          ...
          {loading ? (
            ...
            <>
              {pinned.slice(0, 2).map(note => (
                <NoteItem key={note.id} note={note} onClick={() => navigate('/notes')} />
              ))}
              {recent.map(note => (
                <NoteItem key={note.id} note={note} onClick={() => navigate('/notes')} />
              ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div className="card">
            ...
            {notes.filter(n => new Date(n.updatedAt).toDateString() === new Date().toDateString()).length === 0 ? (
              <p style={{ fontSize: 13, color: 'var(--text-muted)', padding: '0.5rem 0' }}>Nothing scheduled for today.</p>
            ) : (
              notes
                .filter(n => new Date(n.updatedAt).toDateString() === new Date().toDateString())
                .slice(0, 4)
                .map(note => (
