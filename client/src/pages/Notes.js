...
export default function Notes() {
  ...
  const openEdit   = (note, e) => { e.stopPropagation(); setEditing(note); setModalOpen(true); };

  const handleSave = async (form) => {
    setSaving(true);
    try {
      if (editing?.id) {
        await notesApi.update(editing.id, form);
  ...
  const handlePin = async (note, e) => {
    ...
    try {
      ...
      setNotes(p => p.map(n => n.id === note.id ? { ...n, isPinned: !n.isPinned } : n));
  ...
  return (
    <AppLayout title="My Notes" subtitle="All your notes in one place">
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)' }}>Loading your notes…</div>
      ) : notes.length === 0 ? (
        <div className="empty-state card">
          <div className="empty-icon">📭</div>
          <p>{search ? `No notes matching "${search}"` : 'No notes yet. Create your first one!'}</p>
          {!search && (
            <button
          ...
          {others.length > 0 && (
            <section>
              <div className="section-header">
                ...
                <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{others.length} notes</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 12 }}>
                {others.map(note => (
                  <NoteCard
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button className="action-btn" onClick={onPin} title={note.isPinned ? 'Unpin' : 'Pin'}>
            📌
          </button>
          <button className="action-btn" onClick={onEdit} title="Edit">
            ✏️
          </button>
          <button className="action-btn delete" onClick={onDelete} title="Delete" disabled={deleting}>
            {deleting ? '…' : '🗑️'}
          </button>
        </div>
      </div>

      <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, display: '-webkit-box', WebkitLineClamp: 4, WebkitBoxOrient: 'vertical', overflow: 'hidden', marginBottom: 12 }}>
        {note.content}
      </p>
